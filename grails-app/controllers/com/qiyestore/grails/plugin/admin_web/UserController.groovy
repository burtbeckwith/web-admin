...
class UserController {
	...
	**/
    def userList() {
		...
		def roles  = TRole.list()

		[params:params,result:result,roles:roles,total:result.totalCount]
	}
	...
	**/
	def userEdit() {
		...
		[params:params,user:user,roles:roles,existStr:existStr,refererUrl:refererUrl]
	}
	...
	***/
	...
	def userSave() {
		...
		String savePath = grailsApplication.config.upfile.savePath
		...
		if(params.roleId){
			...
			if(role){
				...
				new TUserTRole(role: role, user: user).save(flush:true)
			}
		}
	...
	****/
	...
	def moduleEdit() {
		...
		[module:module,parentModule:parentModule,refererUrl:refererUrl]
	}
	...
	def moduleSave() {
		...
		if(module.hasErrors()){
			log.error "module save error: $module.errors"
			result ="fail"
		}
	...
	def roleList() {
		...
		[params:params,result:result,total:result.totalCount]
	}
	...
	def roleEdit() {
		...
		if(modules){
			modules.each{
				...
				if(roleModules){
					checked = roleModuleStr.indexOf(",${it.id},") > -1
				}
       ...
       [treeJson:treeJson as JSON,role: role,refererUrl:refererUrl]
    }
    ...
    ***/
		...
		if(rm){
			...
			for(int i=0;i<rm.length;i++){
				...
				if(moduleId){
					...
					if(module){
						def t = new TRoleTModule(module: module, role: role)
						t.save(flush:true)
						if(t.hasErrors()){
							log.debug t.errors
						}
    ...
    def moduleList() {
		...
		[modules:modules,parentModule:parentModule,treeJson: treeJson as JSON]
	}

    private getChildNodes(module){
